...
>({
  ...
  // React 18 compat: prepareUpdate is required (React 19 removed it)
  prepareUpdate(
    _instance: DOMElement,
    _type: ElementNames,
    oldProps: Props,
    newProps: Props,
  ): { oldProps: Props; newProps: Props } | null {
  ...
  // React 18 ConcurrentRoot requires getCurrentEventPriority to determine
  // scheduling priority. DefaultEventPriority (32) matches React's default.
  ...
  // React 18 compat: commitUpdate receives updatePayload from prepareUpdate as 2nd arg
  // React 19 receives old and new props directly (no updatePayload)
  commitUpdate(
    node: DOMElement,
    updatePayloadOrType: { oldProps: Props; newProps: Props } | ElementNames,
    typeOrOldProps: ElementNames | Props,
    oldPropsOrNewProps: Props,
    newPropsOrInternalHandle?: Props,
  ): void {
    // Detect React 18 vs 19 calling convention
    let oldProps: Props
    let newProps: Props
    if (updatePayloadOrType && typeof updatePayloadOrType === 'object' && 'oldProps' in updatePayloadOrType) {
      // React 18: commitUpdate(instance, updatePayload, type, oldProps, newProps, internalHandle)
      oldProps = oldPropsOrNewProps
      newProps = newPropsOrInternalHandle!
    } else {
      // React 19: commitUpdate(instance, type, oldProps, newProps)
      oldProps = typeOrOldProps as Props
      newProps = oldPropsOrNewProps
    }
